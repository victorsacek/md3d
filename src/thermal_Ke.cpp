...
//extern PetscInt *indice_aux_vec_ele;
...
PetscErrorCode montaKeThermal_simplif(double *Ke_local,double *Ke){
	...
	a_aux[1]= vv[1]*dy_const/(kappa*2.0);
	a_aux[2]= vv[2]*dz_const/(kappa*2.0);
			...
			for (kx=-r06; kx<=r06; kx+=r06){
				...
				for (i=0;i<V_NE;i++){
					...
					for (j=0;j<V_NE;j++){
						...
						/*Ke[i*V_NE+j]+=  prodH*volume*N[i]*(N_x[j]*vv_local[0]+N_y[j]*vv_local[1]+N_z[j]*vv_local[2]);
						 
						 if (mod_vv>0) Ke[i*V_NE+j]+= prodH*volume*
						 (kappa_arti*(N_x[i]*unit_vv[0]+N_y[i]*unit_vv[1]+N_z[i]*unit_vv[2])/mod_vv)*
						 (N_x[j]*vv_local[0]+N_y[j]*vv_local[1]+N_z[j]*vv_local[2]);*/
						
						Ke[i*V_NE+j]+= kappa*prodH*volume*(N_x[i]*N_x[j]+N_y[i]*N_y[j]+N_z[i]*N_z[j]);
					}
				}
